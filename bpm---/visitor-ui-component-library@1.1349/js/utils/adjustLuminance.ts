...
export const adjustLuminance = (colorHex, luminanceShiftPercentage) => {
  ...
  const newRedColor = 0 | (1 << 8) + r + (256 - r) * luminanceShiftPercentage / 100;
  const redHex = `0${newRedColor.toString(16).substr(1)}`.substr(-2);
  const newGreenColor = 0 | (1 << 8) + g + (256 - g) * luminanceShiftPercentage / 100;
  const greenHex = `0${newGreenColor.toString(16).substr(1)}`.substr(-2);
  const newBlueColor = 0 | (1 << 8) + b + (256 - b) * luminanceShiftPercentage / 100;
